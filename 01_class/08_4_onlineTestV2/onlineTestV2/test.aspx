...
<body>
    ...
    <div>
        ...
        號同學:安好<br />
        本測驗共10題，每題10分總計100分<br />
        準備好了嗎?<asp:Button ID="Button1" runat="server" Text="開始測驗" OnClick="Button1_Click" />
    
        <br />
    
        第<asp:Label ID="No" runat="server" Text=""></asp:Label>題:
        <asp:Label ID="add1" runat="server" Text="被加數"></asp:Label>+
        <asp:Label ID="add2" runat="server" Text="加數"></asp:Label>加數=
        <asp:TextBox ID="answer" runat="server"></asp:TextBox>
        <asp:Button ID="Button2" runat="server" Text="確認繳交" OnClick="Button2_Click" />
        <br />
        <asp:Label ID="message" runat="server" Text=""></asp:Label>

        <br />

        <asp:Button ID="go_back" runat="server" Text="結束測驗" OnClick="Button3_Click" />
        <br />
        <asp:Label ID="total" runat="server" Text=""></asp:Label>
        <br />
        <asp:Label ID="question_total" runat="server" Text=""></asp:Label>
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:insert_test %>" DeleteCommand="DELETE FROM [test] WHERE [學號] = ?" InsertCommand="INSERT INTO [test] ([學號], [測驗學科], [測驗日期], [測驗成績]) VALUES (?, ?, ?, ?)" ProviderName="<%$ ConnectionStrings:insert_test2.ProviderName %>" SelectCommand="SELECT * FROM [test]" UpdateCommand="UPDATE [test] SET [測驗學科] = ?, [測驗日期] = ?, [測驗成績] = ? WHERE [學號] = ?">
            <DeleteParameters>
